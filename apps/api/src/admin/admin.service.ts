...
export class AdminService {
  ...
  async getDashboard() {
    const [products, clientProducts, orders, users] = await Promise.all([
      this.prisma.product.count({ where: { type: 'SHOP' } }),
      this.prisma.product.count({ where: { type: 'CLIENT' } }),
  ...
  async getAllProducts(params: { page?: number; limit?: number; type?: string; status?: string }) {
